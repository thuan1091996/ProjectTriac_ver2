...
//    ICR1H=0xF9FF>> 8;           //Top value
//    ICR1L=0xF9FF& 0xFF;
//    OCR1AH=0>> 8;          //Count value
//    OCR1AL=0& 0xFF;
//----------------Timer2--------------------------------//
...
{
    if(T_shift_delay>=10)            //sau 8*80us ve diem 0
    return 1;                       //Ve diem 0
    else                            //Chua ve diem 0  
    return 0; 
};
...
void Display()  //TEST DONE
{
    ...
    Dvi=MA_7SEG[(count)%10];
    Chuc=MA_7SEG[(count)/10];
...
void Enable_PWM(void)
{
    ...
    TCCR1B=(0<<ICNC1) | (0<<ICES1) | (1<<WGM13) | (1<<WGM12) | (0<<CS12) | (0<<CS11) | (1<<CS10);
    TIMSK=(1<<TICIE1);
    Load=(count*(0xF9FF))/100; 
    ICR1H=0xF9FF>> 8;           //Top value
    ICR1L=0xF9FF& 0xFF;
    OCR1AH=Load>> 8;          //Count value
    OCR1AL=Load& 0xFF;
};
/*--------------------------------------------------------*/
